...
- The Cycle frontmost behavior (`FrontmostTargetBehavior.cycleWindows`) rotates through the target's current-Space windows (minimized included) on repeat presses. `WindowCycleCoordinator` owns the cursor: identity is always a `CGWindowID` against a sorted-wid rotation order, never an index into `kAXWindows` (that order reshuffles after a raise). A live session cursor outranks the AX focused-window report because focus reporting lags rapid presses; the session invalidates on frontmost change, target termination, pid change, and idle expiry. Cycle presses use a shorter per-bundle cooldown (150ms) gated by a workspace-snapshot frontmost pre-check that runs before the AX lane decision; the same e2e + physical-key verification rule applies to that value. Fewer than two cyclable windows falls back to standard toggle semantics so the shortcut keeps its "step aside" ability. Cycle creates no coordinator sessions (the app stays frontmost).
