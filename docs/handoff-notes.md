...
## Current State
On 2026-04-18, issue #175 added the first full Sparkle update path on the `issue-175-sparkle-auto-update` line: `AppController` now owns a shared `SparkleUpdateService`, `AppPreferences` and `GeneralTabView` now expose a manual `Check for Updates…` action plus update-default presentation, `scripts/package-app.sh` now embeds and signs `Sparkle.framework`, `scripts/package-update-zip.sh` packages Sparkle archives, `scripts/generate-appcast.sh` generates `appcast.xml`, and `.github/workflows/release.yml` now targets a dual-track release model with DMG publication on GitHub Releases plus Sparkle ZIP/appcast publication to Cloudflare R2. This work was prepared from a Linux editing session without a Swift toolchain, so no local `swift build` / `swift test` / packaging run happened there, and no macOS Sparkle runtime validation has been completed yet.
...
### 2026-04-18
- Static review completed for the Sparkle integration branch changes covering `Package.swift`, `SparkleUpdateService`, packaging scripts, and the CI/release workflows
- A new regression test was added for `SparkleUpdateService`'s Info.plist default fallback when `SUFeedURL` / `SUPublicEDKey` remain unset, but it could not be executed in this Linux session because `swift` is unavailable here
- Local command execution could not run `swift build`, `swift test`, `bash scripts/package-app.sh`, `bash scripts/package-update-zip.sh`, or `bash scripts/generate-appcast.sh` because this editing environment lacks the macOS Swift toolchain and macOS packaging tools
- macOS runtime validation for Sparkle feed retrieval, background download behavior, archive replacement, and post-update relaunch remains pending
...
- `bash scripts/e2e-full-test.sh` passed on packaged `build/Quickey.app` with the current real Hyper-only fixture (`All 6 tests passed`, `0 warnings`)
...
- Sparkle release validation on macOS, including `Sparkle.framework` loading from the packaged app, manual `Check for Updates…`, background download behavior, archive replacement, and relaunch after install
- Credential-backed validation of Cloudflare R2 publication plus final `appcast.xml` activation ordering
...
- Sparkle packaging and release automation are now implemented on the issue #175 branch, but they have not yet been compiled or exercised on macOS; the current risk surface includes SwiftPM binary-artifact embedding, nested Sparkle signing, `generate_appcast` execution, R2 upload wiring, and real updater runtime behavior
- Signed/notarized release validation now depends on both Developer ID credentials and the Sparkle/R2 secret set; until those credentials exist, only the internal-package DMG artifact path can be verified end-to-end

## Immediate Next Actions
1. Run `swift build`, `swift test`, `bash scripts/package-app.sh`, `bash scripts/package-update-zip.sh`, and `bash scripts/generate-appcast.sh` on a macOS machine with Swift installed, then fix any compile, package-resolution, or artifact-path issues that surface
2. Validate the packaged app on macOS from `/Applications/Quickey.app`, including the new General-tab `Check for Updates…` action and the default automatic-check/automatic-download behavior
3. Publish a test Sparkle feed to R2 and verify ZIP download, archive replacement, relaunch, and version bump end-to-end before treating issue #175 as complete
4. Keep the intended shortcut fixture explicit before interpreting readiness logs. Read the live `shortcuts.json` first, then match the expected readiness snapshot (`carbon=true`, `eventTap=true`, or both) to that saved route mix instead of assuming the old Safari/IINA baseline.
5. Expand the targeted macOS validation matrix from the now-confirmed Safari fresh-launch / relaunch / external-activation paths to Finder, Terminal, Home, Clock, System Settings, hidden/minimized paths, and event-tap timeout stress
6. Verify standard-shortcut vs Hyper parity on the same target apps beyond the already revalidated Safari cases, and capture any remaining app-specific exceptions in this file
7. Run the full release workflow with real Developer ID, Sparkle, and R2 credentials on a `v*` tag, then validate both the published DMG and the published Sparkle feed on a clean macOS machine
8. Fold any new validation findings back into this note, not into the feature overview docs
