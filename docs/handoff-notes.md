...
On 2026-04-18, issue #183 ("rename Quickey to Wink") was evaluated and explicitly deferred as a product decision checkpoint rather than executed immediately. A dedicated evaluation note now captures the rationale and migration guardrails (`docs/archive/issue-183-wink-rename-evaluation.md`): rename benefits are acknowledged, but current priority remains runtime reliability and macOS validation continuity, while a rename would introduce multi-surface identity migration risk (bundle id, TCC, login item identity, and persisted paths). No product/bundle/path rename has been applied in this cycle.
