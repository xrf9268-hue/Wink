...
## Current State
On 2026-04-21, issue #180 upgraded the menu bar dropdown from static actions only to a lightweight shortcut overview panel. `MenuBarController` now rebuilds a dynamic section above the existing Settings / Launch at Login / Quit items every time the menu opens, grouped under a non-interactive `SHORTCUTS` header and driven by the shared `ShortcutStatusProvider` semantics from #178 so the menu reflects saved shortcut order, shortcut glyphs, running dots, disabled dimming, and unavailable-app warning state without turning the menu into a second settings page. Verification includes dedicated presentation/menu rebuild tests, a full local `swift test` pass, and a same-day packaged-app macOS spot-check via `System Events` confirming the dropdown exposes the current enabled shortcut set with the expected header, order, shortcut text, and running indicators. The unavailable/disabled visual states are covered by code-level tests, but were not forced in the live app because the current local shortcut fixture did not contain those states.
