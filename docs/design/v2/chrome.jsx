// Chrome v2 — window + popover + sidebar + card + banner.
...
// Popover with a real triangle notch
...
function Sidebar({ t, active, onPick }) {
  const items = [
    { id: 'shortcuts', label: 'Shortcuts', icon: window.Icons.keyboard },
  ...
  return (
    <div style={{
      ...
      // Extra top padding so the first row clears the title bar cleanly,
      // matching System Settings' breathing room below the traffic lights.
      padding: '14px 10px 12px',
      display: 'flex', flexDirection: 'column', gap: 2,
    }}>
      {items.map((it) => {
        const isActive = active === it.id;
        // macOS sidebar: line-art glyph only, no colored tile behind it.
        // Inactive = secondary label; active = accent tint on the glyph,
        // with the row-wide selection fill carrying the "selected" signal.
        const iconColor = isActive ? t.accent : t.textSecondary;
        return (
    ...
    <div style={{
      ...
      {title && (
        <div style={{
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: t.textPrimary, flex: 1, letterSpacing: -0.1 }}>{title}</div>
          {accessory}
        </div>
      )}
      {children}
...
function Banner({ t, kind = 'info', title, body, trailing }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: palette.fg, flexShrink: 0, marginTop: 1 }}>{palette.icon(palette.fg)}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: palette.fg, marginBottom: 2 }}>{title}</div>
        {body && <div style={{ fontSize: 11, color: t.textSecondary, lineHeight: 1.45 }}>{body}</div>}
      </div>
